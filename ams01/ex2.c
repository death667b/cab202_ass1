...
void setup(void) {
    ...
    //      initial location which is displaced 14 units to the left 
    //      and 8 units above the star.
    char * station_text = "9";
    station = sprite_create(x_star-14, y_star-8, 1, 1, station_text);
    ...
    double dx = abs(x_star - sprite_x(station));
    ...
    //      star and that of the station.
    double dy = abs(y_star - sprite_y(station));
